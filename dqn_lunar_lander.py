...
def plot_rewards(show_result=False):
    plt.figure(1)
    rewards_t = torch.tensor(episode_cum_rewards, dtype=torch.float)
    if show_result:
        plt.title('Result')
    else:
        plt.clf()
    ...
    plt.savefig("lunar_lander_dqn.png")
...
for i_episode in tqdm.tqdm(range(num_episodes)):
    ...
    for t in count():
        ...
        if done:
            episode_cum_rewards.append(episode_reward)
            if i_episode % 10 == 0:
                print(f'Mean reward: {sum(episode_cum_rewards[-10:])/10}')
            break
...
def test_model():
    policy_net = DQN(n_observations, n_actions).to(device)
    policy_net.load_state_dict(torch.load("lunar_lander_dqn.pth"))
    env = gym.make("LunarLander-v3", continuous=False, gravity=-10.0,
    ...
    for i in range(10):
        ...
        while not done:
            ...
            obs, rew, done, truncated, info = env.step(action.item())
            rewards.append(rew)
            env.render()
